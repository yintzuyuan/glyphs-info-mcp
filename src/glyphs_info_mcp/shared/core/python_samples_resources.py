#!/usr/bin/env python3
"""
Python Samples Resource Manager
Manages Glyphs SDK Python Samples as MCP resources (Issue #37)
"""
...
class PythonSamplesResourceManager:
    """Python Samples Resource Manager

    Manages Python plugin samples as MCP resources.
    ...
    def scan_python_samples(self) -> dict[str, dict[str, Any]]:
        """Scan all Python plugin samples

        Reuses SDKNativeAccessor.list_python_samples() - no need to reimplement scanning.

        Returns:
            Sample dictionary, key is sample ID, value is sample info
        """
        samples = {}
        python_samples = self.native_accessor.list_python_samples()
        ...
        for sample in python_samples:
            ...
            samples[sample_id] = {
                ...
                "type": sample["type"],
                "path": sample["path"],
                "has_bundle": sample.get("has_bundle", False),
    ...
    def _generate_sample_id(self, sample_name: str) -> str:
        ...
        Examples:
        - "Callback for context menu" → "callback_for_context_menu"
    ...
    def get_samples(self) -> dict[str, dict[str, Any]]:
        ...
        if self._samples_cache is None:
            self._samples_cache = self.scan_python_samples()
    ...
    def get_sample_by_id(self, sample_id: str) -> dict[str, Any] | None:
        ...
        Uses native_accessor.get_python_sample() to fetch complete content.
        ...
        full_sample = self.native_accessor.get_python_sample(sample_meta["name"])
    ...
    def get_resource_list(self) -> list[dict[str, str]]:
        ...
        for sample_id, info in samples.items():
            ...
            description = info.get("readme", "Python plugin sample")[:100]
            ...
            resources.append(
                {
                    ...
                    "name": f"Python Sample: {info['name']}",
