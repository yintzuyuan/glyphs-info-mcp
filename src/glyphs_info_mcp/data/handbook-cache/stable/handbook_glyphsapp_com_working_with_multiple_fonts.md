## [Working with Multiple Fonts](#working-with-multiple-fonts)

### [Grouping Fonts into Families](#grouping-fonts-into-families)

A *font* typically refers to a single font file, like *Regular*, *Bold*, or *Semibold Italic*.
...
Use style linking as described in [Style Linking](https://handbook.glyphsapp.com/single-page/#style-linking "Font Info ⯈ Exports ⯈ Style Linking") to link the bold and italic styles.
This enables the `Bold` and `Italic` buttons and the ⌘`B` and ⌘`I` keyboard shortcuts common in many applications.
...
### [Glyphs Files, Masters, & Instances](#glyphs-files-masters-instances)
...
Convert a single instance to a master by selecting the instance in `File → Font Info… → Exports` and choosing `Instance as Master` from the plus ![](https://handbook.glyphsapp.com/v/content/icons/plus-button.svg) menu.
The master will be added to the `Masters` tab.
Copy a master from an open Glyphs file into the currently open file by choosing `Add Other Master` from the plus ![](https://handbook.glyphsapp.com/v/content/icons/plus-button.svg) menu in `File → Font Info… → Masters`.

### [Compare Fonts](#compare-fonts)
...
These categories can be collapsed or expanded by clicking the disclosure ![](https://handbook.glyphsapp.com/v/content/icons/disclosure-chevron.svg) triangle next to the gray headings.
...
### [Compare Family](#compare-family)

Open `Edit → Compare Family…` for a comparison of all open documents.
Each document is represented by a column:

![](https://handbook.glyphsapp.com/v/content/images/root/compare-family-left-kerning-group.png)
...
![](https://handbook.glyphsapp.com/v/content/images/root/compare-family-glyphs.png)

Double-click a cell to open the corresponding glyph in Edit View.
Click the filter ![](https://handbook.glyphsapp.com/v/content/icons/filter.svg) button to filter the list of glyphs.
...
Click the pin ![](https://handbook.glyphsapp.com/v/content/icons/pin.svg) button to keep the window above all Glyphs document windows.
