...
![](https://handbook.glyphsapp.com/v/content/images/root/switching-shapes.svg)

Some glyphs may not stay compatible as they interpolate.
Examples include the dollar sign (`$`) losing its stroke in the middle in bold weights or double-story forms switching to a single-story form at an italic angle (`a` → `a` and `g` → `g`).
...
![](https://handbook.glyphsapp.com/v/content/images/root/palette-layers-alternate-layers.svg)

Alternate layers are added to individual glyphs.
For each master, select the master layer in the `Layers` palette, click the plus ![](https://handbook.glyphsapp.com/v/content/icons/plus.svg) button to add a new layer, `Control`-click or right-click the new layer and choose `Alternate` from the context menu.

![](https://handbook.glyphsapp.com/v/content/images/root/alternate-layer-settings-popover.png)
...
This is why Alternate layers are also referred to as *bracket layers*.
...
The [Master Compatibility view](https://handbook.glyphsapp.com/single-page/#master-compatibility "Interpolation ⯈ Outline Compatibility ⯈ Master Compatibility") also shows Alternate layers, but they are offset by a gap:
...
![](https://handbook.glyphsapp.com/v/content/images/root/master-compatibility-alternate-layers.svg)
...
![](https://handbook.glyphsapp.com/v/content/images/root/palette-layers-alternate-master-layer.svg)
...
### [Replacing Glyphs at Export](#replacing-glyphs-at-export)
...
![](https://handbook.glyphsapp.com/v/content/images/root/rename-glyphs-dialog.png)
...
### [Conditional Glyph Substitutions](#conditional-glyph-substitutions)

Conditional substitutions allow glyphs to be replaced in variable fonts.
See [Conditional Feature Code](https://handbook.glyphsapp.com/single-page/#layout/conditional-feature-code "Feature Code ⯈ Conditional Feature Code") for details.
...
When exporting to both variable fonts and static instances, consider using the glyph replacement described in [Replacing Glyphs at Export](https://handbook.glyphsapp.com/single-page/#replacing-glyphs-at-export "Interpolation ⯈ Switching Shapes ⯈ Replacing Glyphs at Export") to mimic the conditional substitutions in static instances.