## [Font-Wide Hints](#postscript-hinting/font-wide-hints)

Before adding glyph-level hinting, define a set of parameters that apply to all hinting throughout the font.
These font-level hints are stored in the so-called *PostScript Private Dictionary* inside the exported font.
For an in-depth discussion, see

- the [Adobe Type 1 Font Format specification](https://adobe-type-tools.github.io/font-tech-notes/pdfs/T1_SPEC.pdf) specifically pages 35–45;
- [Robothon 2012: Postscript hints](https://vimeo.com/38364880) approximately 35 min, a video presentation about PostScript hinting by Miguel Sousa from Adobe.

### [Standard Stems](#postscript-hinting/standard-stems)

Stem widths are the thicknesses of letter strokes.
A *vertical* stem is the width of a vertical stroke of a letter, for example, the thickness of the `I`, or the thicknesses of left and right curves of an `O`.
A *horizontal* stem is the thickness of a horizontal stroke movement, for example, the serifs or crossbars of `A` and `H`, or `t` and `f`, or the upper and lower curves of an `O`.

*Standard* stems are average values, as representative as possible, for as many stem widths in the font as possible.
...
See [Stems](https://handbook.glyphsapp.com/single-page/#masters/stems "Font Info ⯈ Masters ⯈ Stems") for details on editing stem values in Font Info.
...
See [Measuring](https://handbook.glyphsapp.com/single-page/#measuring "Edit View ⯈ Measuring").
...
The horizontal stems also play a role in [TrueType hinting](https://handbook.glyphsapp.com/single-page/#truetype-hinting "TrueType Hinting").
...
### [Alignment Zones](#postscript-hinting/alignment-zones)
...
This difference, usually some ten to fifteen units, is commonly referred to as *overshoot*.
...
The position is sometimes also referred to as the *flat edge* of a zone.
...
Such zones are referred to as *top zones*.
If, however, the overshoots extend below the position (typically for the baseline or descender), the size must be negative and is referred to as a *bottom zones*.
See [Metrics & Alignment Zones](https://handbook.glyphsapp.com/single-page/#masters/metrics "Font Info ⯈ Masters ⯈ Metrics & Alignment Zones") for details on editing the alignment zones of a font master.
...
More precisely, the maximum size of an alignment zone is constrained by the `blueScale` value (see below), which implies that no zone must be larger than 240 ÷ (240 × `blueScale` − 0.98).
...
[Grid Spacing & Subdivision](https://handbook.glyphsapp.com/single-page/#grid-spacing-and-subdivision "Font Info ⯈ Other Settings ⯈ Grid Spacing & Subdivision")), extend the scope of the zones by one unit in both directions to catch potential small rounding errors for vertical node positions.
...
### [Custom Parameters](#postscript-hinting/custom-parameters)
