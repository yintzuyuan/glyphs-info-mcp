...
These features are described in the OpenType specification for the [`GSUB` table](https://docs.microsoft.com/typography/opentype/spec/gsub) (glyph substitution) and the [`GPOS` table](https://docs.microsoft.com/typography/opentype/spec/gpos) (glyph positioning).
...
See [Implicit Features](https://handbook.glyphsapp.com/single-page/#layout/implicit-features "Font Info ⯈ Features ⯈ Implicit Features") for details.

### [Editing Feature Code](#layout/code)

OpenType features are defined using the AFDKO (Adobe Font Development Kit for OpenType) syntax.
Glyphs also offers [additional feature code functionality](https://handbook.glyphsapp.com/single-page/#layout/feature-code "Feature Code").

The `Features` sidebar is divided into three parts:
`Prefix`, `Classes`, and `Features`.
Click the plus ![](https://handbook.glyphsapp.com/v/content/icons/plus.svg) button located to the right of each heading to add an entry.
Select one or multiple sidebar entries and delete them by clicking the minus ![](https://handbook.glyphsapp.com/v/content/icons/minus-button.svg) button located in the bottom-left of the window.
...
- `Classes` are lists of glyph names.
  The names are separated by a white space character, either a space or a line break.
- `Features` control the glyph substitution and positioning rules.
  A feature is identified by its four-character tag.
  Click the plus ![](https://handbook.glyphsapp.com/v/content/icons/plus.svg) button next to the `Feature` heading and search by feature tag or feature name.
...
### [Automatic Feature Code](#layout/automatic-code)

Glyphs can automatically generate feature code for many commonly used OpenType features.
Click the ![](https://handbook.glyphsapp.com/v/content/icons/arrow-rotating-top-cw.svg) `Update` button located in the bottom-left of the window to create and update automatically generated prefixes, classes, and features all at once.
...
Automatic prefixes, classes, and features are indicated with an update ![](https://handbook.glyphsapp.com/v/content/icons/arrow-rotating-top-cw.svg) button to the right of their name.
...
See [Manual Feature Code](https://handbook.glyphsapp.com/single-page/#layout/manual-code "Font Info ⯈ Features ⯈ Manual Feature Code") for details.
...
The *Capital Spacing* feature (`cpsp`), which adds a bit of extra space between capital letters, can also be automatically generated but is not required by all designs, and some designers might want to write it manually.

Some features automatically generated by Glyphs do not appear in the `Features` tab.
See [Implicit Features](https://handbook.glyphsapp.com/single-page/#layout/implicit-features "Font Info ⯈ Features ⯈ Implicit Features") for details.

### [Manual Feature Code](#layout/manual-code)
...
Feature code is written in the AFDKO syntax, as specified by the [OpenType Feature File Specification](https://adobe-type-tools.github.io/afdko/OpenTypeFeatureFileSpecification.html).
Glyphs additionally supports [extended feature code syntax](https://handbook.glyphsapp.com/single-page/#layout/feature-code "Feature Code").

After editing feature code, click the `Compile` button located in the bottom-left of the window to preview it in Edit View.
See [Previewing OpenType Features](https://handbook.glyphsapp.com/single-page/#previewing-opentype-layout "Edit View ⯈ Previewing & Testing ⯈ Previewing OpenType Features") for details.
...
![](https://handbook.glyphsapp.com/v/content/images/root/feature-code-autocompletion.svg)
...
![](https://handbook.glyphsapp.com/v/content/images/root/feature-class-preview.svg)
...
Click the ![](https://handbook.glyphsapp.com/v/content/icons/error.svg) error icon for an expanded description.
Errors with a wrench ![](https://handbook.glyphsapp.com/v/content/icons/error-fix.svg) icon include a suggested fix.
Click the `Fix` button to accept the suggestion.

![](https://handbook.glyphsapp.com/v/content/images/root/features-code-verification.svg)

Prefixes, classes, and features with errors show an ![](https://handbook.glyphsapp.com/v/content/icons/error.svg) icon in the sidebar.
Click the error icon in the sidebar to jump directly to the erroneous part of the code.

![](https://handbook.glyphsapp.com/v/content/images/root/feature-notes.png)
...
Click the snippets ![](https://handbook.glyphsapp.com/v/content/icons/snippets-button.svg) button to insert predefined code snippets.
Choose any of the snippets to insert them into the feature code.
See [Custom Feature Code Snippets](https://handbook.glyphsapp.com/single-page/#custom-feature-code-snippets "Appendix ⯈ Custom Feature Code Snippets") for details on adding custom feature code snippets.

### [Naming Stylistic Sets](#layout/naming-stylistic-sets)
...
Click the plus ![](https://handbook.glyphsapp.com/v/content/icons/plus.svg) button located to the right of a row to add additional localizations of a feature name.
Click the minus ![](https://handbook.glyphsapp.com/v/content/icons/minus.svg) button to remove a row.
...
### [Implicit Features](#layout/implicit-features)
...
Kerning values are defined in Edit View and managed by the Kerning window (see [Kerning](https://handbook.glyphsapp.com/single-page/#kerning "Spacing & Kerning ⯈ Kerning")).
...
:   `mark` is generated if base glyphs have base anchors such as `top` or `bottom` and mark glyphs have matching mark anchors such as `_top` or `_bottom`.
    ...
    See [Mark to Base Positioning](https://handbook.glyphsapp.com/single-page/#mark-to-base-positioning "Edit View ⯈ Anchors ⯈ Mark to Base Positioning") for details.

*Mark to mark*
:   `mkmk` is generated similarly to the `mark` feature.
    See [Mark to Mark Positioning](https://handbook.glyphsapp.com/single-page/#mark-to-mark-positioning "Edit View ⯈ Anchors ⯈ Mark to Mark Positioning").

*Cursive positioning*
:   `curs` is also defined in Edit View by placing special `exit` and `entry` anchors.
    See [Cursive Attachment](https://handbook.glyphsapp.com/single-page/#cursive-attachment "Edit View ⯈ Anchors ⯈ Cursive Attachment").

Implicit features can also be extended manually.
Click the plus ![](https://handbook.glyphsapp.com/v/content/icons/plus.svg) button next to the `Features` heading in the sidebar and search for a feature tag, for example, `kern`.
...
Insert the automatic code placeholder by clicking the snippets ![](https://handbook.glyphsapp.com/v/content/icons/snippets-button.svg) button located in the bottom-right of the window.

### [Export-Specific Features](#layout/export-specific-features)
