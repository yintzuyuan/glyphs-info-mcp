...
Some scripts require specific modules to be installed; install these from the `Modules` tab in the [Plugin Manager](https://handbook.glyphsapp.com/single-page/#plugin-manager "Extensions ⯈ Plugin Manager").
...
See [Shortcuts](https://handbook.glyphsapp.com/single-page/#settings/shortcuts "Settings ⯈ Shortcuts") for details.
Re-run the last script with `Cmd-Opt-R` or from the bottom of the `Script` menu.

### [The Scripts Folder](#scripts/folder)
...
### [Creating Scripts](#scripts/create)
...
For writing the actual script, consult the [Glyphs Python API documentation](https://docu.glyphsapp.com).
On the Glyphs website, there is a [tutorial series](https://glyphsapp.com/learn/recommendation:learn-python) from the first steps of programming in Python up to writing advanced scripts.
