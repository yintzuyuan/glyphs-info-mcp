...
Click the plus ![](https://handbook.glyphsapp.com/v/content/icons/plus-button.svg) button located in the bottom-left of the `Exports` tab to add a new static or variable instance.
Four options are available:

- `Add Instance` adds a new instance named `Regular`.
- `Add Instance for Each Master` adds instances at the designspace locations of each master.
  For example, if the font has two masters, *Thin* with a weight of 100 and *Bold* with a weight of 700, a *Thin* and a *Bold* instance will be added with the same axis values as the respective masters.
- `Instance as Master` adds a new master to the `Masters` tab using the outlines and metrics of the currently selected instance.
  For example, consider a font with two masters—*Thin* and *Bold*—and five instances—*Thin*, *Light*, *Regular*, *Semibold*, and *Bold*—where the Light, Regular, and Semibold instances are interpolations between the two masters.
  Select the Regular instance and choose `Instance as Master` to add a *Regular* master with the outlines and metrics taken from the instance.
  Now the Light instance interpolates between the Thin and Regular masters, the Regular instance uses the Regular master without interpolation, and the Semibold instance interpolates between the Regular and Bold masters.
- `Add Variable Font Setting` adds a variable font instance.
  See [Exporting Variable Fonts](https://handbook.glyphsapp.com/single-page/#export/variable-fonts "Import & Export ⯈ Exporting Font Files ⯈ Exporting Variable Fonts") for more information.

Click the minus ![](https://handbook.glyphsapp.com/v/content/icons/minus-button.svg) button to delete the selected instances.
...
For more information on interpolating instances, see [Interpolation](https://handbook.glyphsapp.com/single-page/#interpolation "Interpolation").
...
See the [Naming tutorial](https://glyphsapp.com/learn/naming) for more information on good style names and how to shorten them for maximum software compatibility.

### [Weight & Width](#exports/weight-and-width)
...
Click the disclosure ![](https://handbook.glyphsapp.com/v/content/icons/pull-down-button.svg) button to select from a range of predefined values.
These values follow the [OpenType `OS/2` specification](https://docs.microsoft.com/typography/opentype/spec/os2): 100, 200, …, 900 for standard weight classes and 1–9 for width classes.
...
Weight and width *classes* are not necessarily the same as the weight and width *axis coordinates* (if a font has such axes).
The weight and width classes are used by applications other than Glyphs to order and categorize fonts.
Axes are used internally in Glyphs for interpolation (though they can also be exposed, see [Setting up Masters: Axes Coordinates](https://handbook.glyphsapp.com/single-page/#interpolation/axes-coordinates "Interpolation ⯈ Setting up Masters ⯈ Axes Coordinates")).
Weight and width axes are not bound to the same value ranges as the weight and width classes.

### [Axes Coordinates](#exports/axes-coordinates)

![](https://handbook.glyphsapp.com/v/content/images/root/instance-in-designspace.svg)

Axes coordinates define the location of an instance in the designspace.
The axes of a font are defined in the `Font` tab (see [Axes](https://handbook.glyphsapp.com/single-page/#font/axes "Font Info ⯈ Font ⯈ Axes")), and masters define the outlines and metrics at specific locations in the designspace of those axes (see [Masters: Axes Coordinates](https://handbook.glyphsapp.com/single-page/#masters/axes-coordinates "Font Info ⯈ Masters ⯈ Axes Coordinates")).

An instance with axis coordinates between masters is *interpolated* and *extrapolated* when it is outside the designspace.
...
Instead, they only allow the user to select a family and its regular, bold, italic, and bold-italic styles using `Bold` and `Italic` buttons.
...
- Other italic styles should always be linked to their non-italic counterpart.
  For example, *Medium Italic* is marked as the italic of *Medium*.

Some font designers also link bolds with other styles.
For example, *Semibold* is marked as the bold of *Light*.
This hides the *Semibold* style from the font family picker, making it only available when pressing the `Bold` button while the *Light* style is selected.
Therefore, users may be unaware that the font family includes a *Semibold* style.

### [Custom Parameters](#exports/custom-parameters)
