## [sbix Fonts](#color-fonts/sbix)

![](https://handbook.glyphsapp.com/v/content/images/root/color-font-apple-color-emoji.png)

See the [TrueType Reference Manual](https://developer.apple.com/fonts/TrueType-Reference-Manual/RM06/Chap6sbix.html) for details on `sbix` color fonts.
...
### [Standard Bitmap Graphics](#color-fonts/sbix/introduction)
...
- Providing images for smaller sizes allows fine-tuning the graphics for low-resolution output.
  For example, the design might be adapted to read more clearly at smaller pixel sizes.
- If smaller images are not included in the font, the text renderer needs to scale the large images down to the desired font size.
  This can be slow, especially on low-end devices, and use more energy, which is relevant to mobile devices.
...
### [Preparing Images](#color-fonts/sbix/preparing-images)
...
### [Adding Images to Glyphs](#color-fonts/sbix/adding-images-to-glyphs)
...
![](https://handbook.glyphsapp.com/v/content/images/root/color-font-sbix-master-layer-setup.svg)

These tiny paths can also have their nodes on the same single point, thus hiding the path.
The red node background ![](https://handbook.glyphsapp.com/v/content/icons/duplicate-nodes-highlight.svg) that Glyphs shows for overlapping nodes can be ignored in this case.
...
Add iColor layers by clicking the plus ![](https://handbook.glyphsapp.com/v/content/icons/plus.svg) button in the `Layers` palette (`Window → Palette`, `Cmd-Opt-P`) and choosing `iColor` from the context menu on the layer.
...
### [Exporting](#color-fonts/sbix/exporting)
...
Glyphs can convert `sbix` color fonts to [SVG color fonts](https://handbook.glyphsapp.com/single-page/#color-fonts/svg "Color Fonts ⯈ SVG Color Fonts").
