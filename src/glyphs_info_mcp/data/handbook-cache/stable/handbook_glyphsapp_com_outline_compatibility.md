## [Outline Compatibility](#interpolation/outline-compatibility)
...
### [Identifying Incompatible Outlines](#identifying-incompatible-outlines)

![](https://handbook.glyphsapp.com/v/content/images/root/incompatible-outlines-glyph-layer.svg)

If the outlines of a glyph are not compatible, a red bar is shown above its ascender line in Edit View and a red corner ![](https://handbook.glyphsapp.com/v/content/icons/non-interpolatable-glyph-cell-indicator.svg) in Font View.
Filter for all incompatible glyphs using [Smart Filters (Glyphs includes an *Incompatible masters* Smart Filter by default)](https://handbook.glyphsapp.com/single-page/#filtering/smart-filters "Font View ⯈ Filtering Font View ⯈ Smart Filters").
...
### [Correcting Path Direction](#correcting-path-direction)
...
See [Controlling Path Direction](https://handbook.glyphsapp.com/single-page/#controlling-path-direction "Edit View ⯈ Editing Paths ⯈ Controlling Path Direction") for more details.

### [Reordering Shapes](#reordering-shapes)
...
See [Shape Order](https://handbook.glyphsapp.com/single-page/#filters/shape-order "Filters ⯈ Built-in Filters ⯈ Shape Order") for details.

### [Master Compatibility](#master-compatibility)
...
![](https://handbook.glyphsapp.com/v/content/images/root/master-compatibility-incompatible.svg)

The following aspects are highlighted in this mode:

- Paths and components are colored based on their shape order.
  Components are additionally displayed with a checkered pattern.
...
- Diagonal lines connect starting nodes, shape centers, and anchors across the masters.
  Select points to show their connecting lines.
  Anchors are connected by dashed lines.
- Path segments are colored either green, yellow, or red.
  Green segments are compatible.
  Yellow segments are compatible, but their angle differs by more than 20° between masters, indicating that a node might be missing.
  Red segments are incompatible; there is either a missing segment in other masters, or the segment types do not match (curved segment vs. straight segment).

Fix path direction and shape order related issues as described in [Correcting Path Direction](https://handbook.glyphsapp.com/single-page/#correcting-path-direction "Interpolation ⯈ Outline Compatibility ⯈ Correcting Path Direction") and [Reordering Shapes](https://handbook.glyphsapp.com/single-page/#reordering-shapes "Interpolation ⯈ Outline Compatibility ⯈ Reordering Shapes").
...
![](https://handbook.glyphsapp.com/v/content/images/root/master-compatibility-missing.svg)

The master layers of a glyph might be compatible but still produce undesirable interpolations.

![](https://handbook.glyphsapp.com/v/content/images/root/shape-shift-setup.svg)
...
![](https://handbook.glyphsapp.com/v/content/images/root/shape-shift.svg)
