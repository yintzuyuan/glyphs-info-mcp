## [Advanced TrueType Hinting](#truetype-hinting/advanced)

Use the following custom parameters to further configure the TT hinting of the exported font files.

##### [TTFOvershootSuppressionBelowPPM](#truetype-hinting/ttfovershootsuppressionbelowppm)
...
The flat edge is the position of a zone as configured in the [`TTFZones` parameter](https://handbook.glyphsapp.com/single-page/#truetype-hinting/zones/position "TrueType Hinting ⯈ Font-level Hints ⯈ TrueType Zones ⯈ Position").

##### [TTZoneRoundingThreshold](#truetype-hinting/ttzoneroundingthreshold)
...
The value is added to any positive zone position before rounding, and added twice to the x-height zone (the one named `xHeight` in the [TrueType zones](https://handbook.glyphsapp.com/single-page/#truetype-hinting/zones/name "TrueType Hinting ⯈ Font-level Hints ⯈ TrueType Zones ⯈ Name")).
...
##### [TTMinimumDistance](#truetype-hinting/ttminimumdistance)
...
##### [Control Value Program](#truetype-hinting/control-value-program)

See the [TrueType Reference Manual](https://developer.apple.com/fonts/TrueType-Reference-Manual/RM03/Chap3.html) for details.
...
##### [gasp Table](#truetype-hinting/gasp-table)
...
- Source for quoted parts: *Now read this: The Microsoft ClearType Font Collection*, Microsoft 2004, p. 14.
  ...
  `No Hinting & Symmetric`:
  Until the first threshold size, no grid-fitting is applied, and text is rendered with antialiasing wherever possible.
...
- `Hinting & Asymmetric`:
  Between the two threshold sizes, the renderer is recommended to apply grid-fitting and suppress grayscale.
  At intermediate sizes, hinting and monochrome rendering will usually produce the best appearance.
  In ClearType, the matter is handled asymmetrically: vertical grid-fitting is applied, while horizontally, subpixel rendering is used.
- `Hinting & Symmetric`:
  Beyond the thresholds, the rasterizer is instructed to apply grid-fitting and render the shapes in grayscale.
  At large sizes, the combination of hinting and grayscale rendering will typically produce the best appearance.
  The ClearType rasterizer is instructed to apply symmetric smoothing.
  This uses antialiasing in the `Y` direction in addition to horizontal subpixel rendering.
