## [Variable Font Options](#variable-font-options)
...
### [Variable Font Origin](#variable-font-origin)

Variable fonts require an *origin master*.
...
This is described in detail in [Minimal Multiple Masters Setup](https://handbook.glyphsapp.com/single-page/#interpolation/masters/minimal-multiple-masters-setup "Interpolation ⯈ Setting up Masters ⯈ Minimal Multiple Masters Setup").
...
![](https://handbook.glyphsapp.com/v/content/images/root/axis-location-dialog-master.png)
...
A common example is the *Weight* axis, which in Glyphs is often based on steam widths, but the OpenType specification [recommends the *Weight* axis to use a range of 1–1000](https://docs.microsoft.com/typography/opentype/spec/dvaraxistag_wght).
Similar recommendations exist for the [*Italic*](https://docs.microsoft.com/typography/opentype/spec/dvaraxistag_ital), [*Optical Size*](https://docs.microsoft.com/typography/opentype/spec/dvaraxistag_opsz), [*Slant*](https://docs.microsoft.com/typography/opentype/spec/dvaraxistag_slnt), and [*Width*](https://docs.microsoft.com/typography/opentype/spec/dvaraxistag_wdth) axis.
...
Further configure the axis coordinates of variable fonts using [axis mappings](https://handbook.glyphsapp.com/single-page/#axis-mappings "Interpolation ⯈ Variable Font Options ⯈ Axis Mappings").

### [Axis Mappings](#axis-mappings)

![](https://handbook.glyphsapp.com/v/content/images/root/axis-mappings-demo.svg)
...
*Linear* in this context means that each axis coordinate is mapped onto itself and is thus unchanged.
...
![](https://handbook.glyphsapp.com/v/content/images/root/axis-mapping-linear.svg)
...
![](https://handbook.glyphsapp.com/v/content/images/root/axis-mapping-non-linear.svg)

The *internal coordinates* are the values set in `File → Font Info… → Masters → Axes Coordinates`.
The *external coordinates* are the result of axis mapping; they are used for interpolation.
...
![](https://handbook.glyphsapp.com/v/content/images/root/axis-mappings-dialog.png)
...
Click the plus ![](https://handbook.glyphsapp.com/v/content/icons/plus-button.svg) button to add a new control point.
...
Delete the selected control point by clicking the minus ![](https://handbook.glyphsapp.com/v/content/icons/minus-button.svg) button.
...
### [Style Attributes Table](#style-attributes-table)
...
Names can be marked as *elidable*.
...
- Add the `Style Name as STAT entry` custom parameter to instances that differ from the [variable font origin](https://handbook.glyphsapp.com/single-page/#variable-font-origin "Interpolation ⯈ Variable Font Options ⯈ Variable Font Origin") on a single axis.
  Set the value to the four-letter tag of that axis.
...
- Add the `Elidable STAT Axis Value Name` custom parameter to instances that have an elidable name component on an axis.
  Set the value to the four-letter tag of the axis on which the instance name is elidable.
  For example, a *Regular* instance might need multiple parameters, one for each font axis, since its name is elidable in every case.
...
Install additional filters from the [Plugin Manager](https://handbook.glyphsapp.com/single-page/#plugin-manager "Extensions ⯈ Plugin Manager").
