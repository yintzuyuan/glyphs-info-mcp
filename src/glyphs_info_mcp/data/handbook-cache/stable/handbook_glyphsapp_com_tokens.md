...
There are three types of tokens: *number values*, *glyph properties*, and *glyph predicates*.

### [Number Value Tokens](#layout/tokens/number-value)
...
### [Glyph Property Tokens](#layout/tokens/glyph-property)
...
Basic arithmetic is supported as in [number value tokens](https://handbook.glyphsapp.com/single-page/#layout/tokens/number-value "Feature Code ⯈ Tokens ⯈ Number Value Tokens").
For example:
...
### [Glyph Predicate Tokens](#layout/tokens/glyph-predicate)
...
:   Check for numeric values of the glyph.
    ...
    For example, `$[colorIndex == 5]` (see [Color Label](https://handbook.glyphsapp.com/single-page/#glyph/color-label "Font View ⯈ Glyph Properties ⯈ Color Label") for color numbers), `$[case == upper]` (use `noCase`, `upper`, `lower`, `smallCaps`, `minor`, or `other` for comparison), or `$[direction == RTL]` (`LTR`, `BIDI`, `RTL`).
