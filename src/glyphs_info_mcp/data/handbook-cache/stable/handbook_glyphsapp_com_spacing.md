...
When the Text tool ![](https://handbook.glyphsapp.com/v/content/icons/text-tool.svg) is active and `View → Show Metrics` (`Cmd-Shift-M`) is checked, the sidebearing values and widths of all glyphs are displayed:

### [Info Box](#spacing/info-box)
...
For left to right and right to left scripts, the sidebearing values are shown to the left and right of the ![](https://handbook.glyphsapp.com/v/content/icons/metrics-horizontal.svg) metrics icon in the center of the Info box:
...
### [Spacing Shortcuts](#spacing/shortcuts)
...
![](https://handbook.glyphsapp.com/v/content/images/root/info-box-horizontal-metrics-keys.svg)

Linked metrics are *not synced automatically* when the metrics of the linked glyphs are changed.
Instead, in Edit View, out-of-sync metrics keys are displayed in red next to an update ![](https://handbook.glyphsapp.com/v/content/icons/arrow-update.svg) button:

![](https://handbook.glyphsapp.com/v/content/images/root/info-box-horizontal-metrics-keys-out-of-sync.svg)

Sync a metric key by clicking its update ![](https://handbook.glyphsapp.com/v/content/icons/arrow-update.svg) button.
...
In Font View, out-of-sync metrics are indicated by a warning ![](https://handbook.glyphsapp.com/v/content/icons/alert.svg) triangle in the top-right corner of the glyph cells.
Consider using a [Smart Filter](https://handbook.glyphsapp.com/single-page/#filtering/smart-filters "Font View ⯈ Filtering Font View ⯈ Smart Filters") to reveal all of the glyphs with out-of-sync metrics.

##### [Metrics Key Calculations](#metrics-keys/calculations)
...
##### [Constant Metrics Keys](#metrics-keys/constant)
...
##### [Mirrored Metrics Keys](#metrics-keys/mirrored)
...
##### [Local Metrics Keys](#metrics-keys/local)
...
##### [Metrics Keys at Baseline Offsets](#metrics-keys/at-baseline-offsets)
...
### [Metrics Keys & Automatic Alignment](#metrics-keys/automatic-alignment)

The metrics fields in the Info box of an [automatically aligned glyph](https://handbook.glyphsapp.com/single-page/#automatic-alignment "Reusing Shapes ⯈ Components ⯈ Automatic Alignment") show the text `auto` and the derived values in parentheses.
