...
Regular expressions (often abbreviated as *regex*) can be used in some search fields to search for text patterns.
...
`\w`
:   Matches *word* characters: letters, digits, and the underscore (`_`).
...
`{n,m}`
:   Like `{n}`, but finds the preceding pattern between *n* and *m* times.
    *n* may be `0` and *m* may be left off not to set an upper limit.
...
See also [Search Field](https://handbook.glyphsapp.com/single-page/#filtering/search-field "Font View ⯈ Filtering Font View ⯈ Search Field").