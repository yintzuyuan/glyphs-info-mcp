...
class TestFileClassifier:
    ...
    def test_get_file_category_binary_stub(self, temp_files: Path):
        """測試分類二進位檔案（Mach-O）"""
        classifier = FileClassifier()
        assert (
    ...
    def test_get_file_category_compiled(self, temp_files: Path):
        """測試分類編譯檔案（pyc, nib）"""
        classifier = FileClassifier()
    ...
    def test_get_language_tag_python(self, temp_files: Path):
        """測試 Python 的語法高亮標籤"""
        classifier = FileClassifier()
        assert classifier.get_language_tag(temp_files / "plugin.py") == "python"

    def test_get_language_tag_objc(self, temp_files: Path):
