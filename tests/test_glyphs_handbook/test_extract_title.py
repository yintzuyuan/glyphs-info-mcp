...
class TestExtractTitle:
    ...
    def test_extracts_h2_title_when_no_h1(self):
        """無一級標題時，應提取二級標題"""
        content = "## [Anchors](#anchors)\n\nAnchors are special points."
        title = EnhancedHandbookSearcher._extract_title(content)
        assert title == "Anchors"

    def test_extracts_h2_with_markdown_link(self):
    ...
    def test_returns_fallback_when_no_heading(self):
        """無任何標題時，返回預設值"""
    ...
    def test_real_handbook_format(self):
        """測試真實 handbook 檔案格式"""
        content = """## [Anchors](#anchors)
